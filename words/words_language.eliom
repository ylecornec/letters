...
let%server best_matched_language () =
  ...
  let rec aux = function
    | (l, _) :: tl -> (
      try Words_i18n.guess_language_of_string l
      with Words_i18n.Unknown_language _ -> aux tl)
    | [] -> Words_i18n.default_language
  in
  aux lang

let%server update_language lang =
  let language = Words_i18n.string_of_language lang in
  let myid_o = Os_current_user.Opt.get_current_userid () in
  (* Update the server and client values *)
  Words_i18n.set_language lang;
  ignore [%client (Words_i18n.set_language ~%lang : unit)];
...
let%server _ =
  ...
  Os_session.on_start_connected_process (fun userid ->
      ...
      let%lwt language =
        ...
        | Some lang ->
            Lwt.return (Words_i18n.guess_language_of_string lang)
        | None ->
            let%lwt best_language = best_matched_language () in
            ignore
              (Os_user.update_language ~userid
                 ~language:(Words_i18n.string_of_language best_language));
            Lwt.return best_language
      in
      Words_i18n.set_language language;
      ignore [%client (Words_i18n.set_language ~%language : unit)];
      Lwt.return_unit)
