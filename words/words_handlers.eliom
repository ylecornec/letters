...
let upload_user_avatar_handler myid () ((), (cropping, photo)) =
  let avatar_dir =
    List.fold_left Filename.concat
      (List.hd !Words_config.avatar_dir)
      (List.tl !Words_config.avatar_dir)
  in
  let%lwt avatar =
...
let%server forgot_password_handler =
  Os_handlers.forgot_password_handler Words_services.settings_service
  ...
  | e ->
      ...
      then
        ...
        then
          ...
          in
          Words_base.App.send
            (Words_page.make_page (Os_page.content page))
        else
          let page =
            [ div
                ~a:[a_class ["login-signup-box"]]
...
let%shared settings_handler myid_o () () =
  let%lwt content =
    match myid_o with
    | Some _ -> Words_settings.settings_content ()
    | None -> Lwt.return [p [%i18n log_in_to_see_page ~capitalize:true]]
  in
  Words_container.page myid_o content

let%server update_language_handler () language =
  Os_session.connected_wrapper Words_language.update_language
    (Words_i18n.language_of_string language)

let%client update_language_handler () language =
  Words_i18n.(set_language (language_of_string language));
